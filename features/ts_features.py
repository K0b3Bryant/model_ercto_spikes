...
def ts_features(df:pd.DataFrame, window:int:3, high, low, close, open, volume) --> pd.DataFrame:
  """ Generates timeseries features """
  ...
  return features.drop(columns=[close, open, high, low, volume], inplace=True)
